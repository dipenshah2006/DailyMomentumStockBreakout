...
LOCAL_NSE_CSV       = os.path.join(SCRIPT_DIR, "nyse-listed.csv")
NSE_CSV_URL         = "https://nsearchives.nseindia.com/content/equities/EQUITY_L.csv"
SERIES_FILTER       = ["EQ"]       # NSE equity series (EQ = cash equities); ignored for NYSE CSV

LOCAL_SME_CSV       = os.path.join(SCRIPT_DIR, "india", "NSE", "NSESME", "MW-SME-05-May-2026.csv")
SME_SERIES_FILTER   = ["ST", "SM"] # NSE SME series (ST = SME T, SM = SME M)
YF_SUFFIX           = ".AX"           # NSE used ".NS"; NYSE uses plain symbols (BRK.B → BRK-B via _yf_ticker)
...
LOCAL_INDICES_XLSX  = os.path.join(SCRIPT_DIR, "india", "NSE", "NIFTY_Indices_Master.xlsx")
...
LOCAL_INDICES_DIR   = os.path.join(SCRIPT_DIR, "india", "NSE", "NseIndice")
LOCAL_FO_CSV        = os.path.join(SCRIPT_DIR, "india", "NSE", "nse_fo_list.csv")   # NSE F&O securities list
...
GITHUB_CHARTS_BASE  = "charts"   # relative path in HTML for lazy-loaded chart images
CHART_OUTPUT_DIR    = os.path.join(SCRIPT_DIR, "charts")   # local folder for generated PNG chart files
...
OUTPUT_HTML = os.path.join(SCRIPT_DIR, "rsi_mtf_report_NYSE.html")   # fixed name — always overwrites
ERROR_LOG   = os.path.join(SCRIPT_DIR, f"error_log_{_STAMP}.txt")   # timestamped per run
CACHE_FILE  = os.path.join(SCRIPT_DIR, "stock_data_cache.pkl")
...
def log_error(ticker: str, company: str, stage: str, exc: Exception):
    ...
    _logger.error(
        f"TICKER='{_yf_ticker(ticker)}' | COMPANY={company!r:35s} | STAGE={stage}\n"
...
def log_warn(ticker: str, company: str, msg: str):
    _logger.warning(f"TICKER='{_yf_ticker(ticker)}' | COMPANY={company!r:35s} | {msg}")
...
def _yf_ticker(symbol: str) -> str:
    """Map exchange ACT symbol to yfinance ticker (e.g. BRK.B → BRK-B)."""
    s = symbol.strip().upper().replace(".", "-")
    return f"{s}{YF_SUFFIX}" if YF_SUFFIX and not s.endswith(YF_SUFFIX) else s


def _is_tradable_equity(symbol: str) -> bool:
    """Skip preferred ($), warrants/units (.W/.U), and other non-common symbols."""
    if not symbol or symbol.isdigit() or len(symbol) > 12:
        return False
    if "$" in symbol:
        return False
    if "." in symbol:
        suffix = symbol.split(".", 1)[1].upper()
        if suffix in {"W", "U", "WS", "RT", "R", "WI", "UN"}:
            return False
    return True
...
def _build_company_map(text: str):
    ...
    # Normalise column names once: strip whitespace and uppercase header keys
    for row in reader:
        clean = {k.strip().upper(): (v.strip() if isinstance(v, str) else v) for k, v in row.items() if k}
        sym = (clean.get("SYMBOL") or clean.get("ACT SYMBOL") or "").strip()
...
def _batch_download(tickers: list[str], period: str | None = None,
    ...
    results = {}
    args = {
        "tickers": [_yf_ticker(t) for t in tickers],
        "interval": "1d",
        "progress": False,
        "auto_adjust": True,
        "group_by": "ticker",
...
def _fetch_mcap_batch(tickers: list[str]) -> dict[str, float]:
    ...
    def _fetch_one(t):
        try:
            info = yf.Ticker(_yf_ticker(t)).fast_info
...
def _parse_nse_csv(text: str, series_filters: list[str], is_sme: bool = False) -> list[str]:
    ...
    is_us_listed = "ACT SYMBOL" in fieldnames and "SYMBOL" not in fieldnames
    tickers = []
    ...
    for row in reader:
        if not row:  # Skip empty rows
            continue
        # Sanitize row keys by stripping whitespace and uppercasing
        clean_row = {}
        for k, v in row.items():
            if k:
                clean_row[k.strip().upper()] = (v.strip() if isinstance(v, str) else v)
        series = clean_row.get("SERIES", "").strip()
        symbol = (clean_row.get("SYMBOL") or clean_row.get("ACT SYMBOL") or "").strip()
        ...
        elif is_us_listed or not series:
            if symbol and _is_tradable_equity(symbol):
                tickers.append(symbol)
        else:
...
def load_universe() -> list[str]:
    ...
    # Fallback to live NSE download only when configured CSV is the NSE equity list
    if os.path.basename(LOCAL_NSE_CSV).lower() != "nyse-listed.csv":
        try:
            s = requests.Session()
        ...
        print(f"  [!] '{LOCAL_NSE_CSV}' missing or empty — cannot download NYSE list automatically")
...
def analyze_stock(ticker: str) -> dict | None:
    ...
    try:
        ...
        if df is None:
            ...
            raw = yf.download(_yf_ticker(ticker), period=DATA_PERIOD, interval="1d",
...
def _html_safe_stock(d: dict) -> dict:
    ...
    if d.get('has_chart'):
        chart_path = os.path.join(GITHUB_CHARTS_BASE, f"{d['ticker']}.png").replace("\\", "/")
